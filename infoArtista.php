<?php 
    ...
    if (!$conexion) {
        ...
        JOIN Album ON idAlbum = Album.id JOIN Artista ON idArtista = Artista.id
        WHERE Cancion.id =
            (SELECT idCancion FROM Usuario_escucha_Cancion 
            WHERE plays = current_date()
            );";

        $resultadoAr = mysqli_query($conexion, $queryArtista);
        $resultadoA = mysqli_query($conexion, $queryAlbum);
